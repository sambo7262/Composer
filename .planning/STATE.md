...
Plan: —
Status: Awaiting next milestone
Last activity: 2026-05-29 — Completed quick task 260529-anf: stop re-analyzing permanently-failed Essentia tracks
...
| 260528-jlg | fix(db): set SQLite `PRAGMA busy_timeout=5000` to eliminate intermittent "database is locked" on concurrent writes. NAS UAT 2026-05-28: Sunday 03:00 UTC weekly maintenance tick failed with "database/table is locked", required manual re-run. WAL mode (already set) separates reader/writer locks but writer-vs-writer contention still fails instantly without `busy_timeout`. 5s timeout lets the loser wait for the lock — each transaction is sub-second so 5s is generous headroom. 1-line addition to existing pragma listener in `app/database.py`; pragma fires on every new connection so timeout applies to all engine paths (pooled + raw). 1 new test (`test_busy_timeout_pragma_is_set_to_5000`) mirroring existing `test_foreign_keys_enabled` pattern; 5/5 pass in test_database.py, 60/61 pass in suggestions+event handlers (1 pre-existing stochastic flake documented in 260517-nkt). | 2026-05-28 | 906c704 | [260528-jlg-fix-db-set-sqlite-pragma-busy-timeout-50](./quick/260528-jlg-fix-db-set-sqlite-pragma-busy-timeout-50/) |
| 260529-anf | fix(analysis): stop re-analyzing Essentia tracks that permanently fail extraction. A failed track set `analysis_error` but left `analyzed_at=None`, so all four "needs analysis" queries (manual queue, post-sync auto-trigger, two pending-count UIs) re-picked the same failing tracks forever. Redefine needs-analysis as `analyzed_at IS NULL AND analysis_error IS NULL` — migration-free, reuses existing column. Since failed tracks are now skipped (and the in-memory error list resets on restart), added DB-backed `get_failed_tracks()` + `error_count` surfaced in the idle/completed banner so failures stay visible, plus `clear_analysis_errors()` and `POST /api/analysis/retry-failed` (+ "Retry failed" button) as the manual re-run path. 500 MB size cap + path remapping untouched. Also fixed pre-existing stale `test_skips_oversized_files` (200MB vs 500MB cap, failing since da076b2). 3 new tests; 26/26 pass in analysis suites. | 2026-05-29 | 398f411 | [260529-anf-stop-retrying-failed-essentia-tracks](./quick/260529-anf-stop-retrying-failed-essentia-tracks/) |
